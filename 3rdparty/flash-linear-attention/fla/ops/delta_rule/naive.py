...
4. delta_rule_parallel_scan: Tree-based Hillis-Steele O(n log n) work, O(log L) depth

Native dtype implementations (preserve fp16/bf16):
Token-level:
5. delta_rule_recurrence_native_dtype: Sequential, O(L) work, O(L) depth
6. delta_rule_parallel_scan_native_dtype: Hillis-Steele, O(L log L) work, O(log L) depth
7. delta_rule_blelloch_scan_native_dtype: Blelloch, O(L) work, O(log L) depth

Chunkwise:
8. delta_rule_chunkwise_native_dtype: Sequential inter-chunk
9. delta_rule_chunkwise_parallel_scan_native_dtype: Hillis-Steele inter-chunk
10. delta_rule_chunkwise_blelloch_scan_native_dtype: Blelloch inter-chunk

Fully parallel:
11. delta_rule_parallel_native_dtype: O(L^2) attention
12. delta_rule_parallel_no_materialize_native_dtype: O(L^2) memory-efficient
"""
...
def delta_rule_blelloch_scan_native_dtype(q, k, v, beta, initial_state=None, output_final_state=True):
    """
    Blelloch parallel scan that preserves input dtype (fp16/bf16).

    Unlike Hillis-Steele which has O(n log n) work, Blelloch has O(n) work
    with O(log n) depth. This is work-efficient.
    ...
    Complexity:
        Time: O(log L) parallel depth, O(L) work
        Space: O(L * d_k^2 + L * d_k * d_v)
    """
    dtype = q.dtype
    device = q.device
    ...
    I = torch.eye(d_k, dtype=dtype, device=device)
    zero_B = torch.zeros(B, H, d_k, d_v, dtype=dtype, device=device)

    # Build A_t = I - β_t * k_t @ k_t^T and b_t = β_t * k_t @ v_t^T
    k_outer = k.unsqueeze(-1) * k.unsqueeze(-2)
    A = I - beta.unsqueeze(-1) * k_outer
    b_mat = beta.unsqueeze(-1) * k.unsqueeze(-1) * v.unsqueeze(-2)

    # Blelloch scan - track both A and B through up-sweep and down-sweep
    A_tree = A.clone()
    B_tree = b_mat.clone()
    ...
    step = 1
    while step < L:
        indices = torch.arange(2 * step - 1, L, 2 * step, device=device)
        if len(indices) == 0:
            break
        src_indices = indices - step

        for idx, src_idx in zip(indices.tolist(), src_indices.tolist()):
            A_idx_old = A_tree[:, :, idx].clone()
            A_tree[:, :, idx] = torch.einsum('bhij,bhjk->bhik', A_idx_old, A_tree[:, :, src_idx])
            B_tree[:, :, idx] = torch.einsum('bhij,bhjk->bhik', A_idx_old, B_tree[:, :, src_idx]) + B_tree[:, :, idx]

        step *= 2

    # Phase 2: Down-sweep
    A_tree[:, :, -1] = I.unsqueeze(0).unsqueeze(0).expand(B, H, -1, -1)
    B_tree[:, :, -1] = zero_B

    step = L // 2
    while step >= 1:
        indices = torch.arange(step - 1, L - 1, 2 * step, device=device)
        if len(indices) == 0:
            break
        ...
        for idx in indices.tolist():
            ...
            if right_idx >= L:
                continue
            # t = left
            A_t = A_tree[:, :, idx].clone()
            B_t = B_tree[:, :, idx].clone()
            # left = right
            A_tree[:, :, idx] = A_tree[:, :, right_idx].clone()
            B_tree[:, :, idx] = B_tree[:, :, right_idx].clone()
            # right = t ∘ old_right
            A_right_old = A_tree[:, :, idx]
            B_right_old = B_tree[:, :, idx]
            A_tree[:, :, right_idx] = torch.einsum('bhij,bhjk->bhik', A_t, A_right_old)
            B_tree[:, :, right_idx] = torch.einsum('bhij,bhjk->bhik', A_t, B_right_old) + B_t

        step //= 2

    # B_tree now contains exclusive prefix sums
    # For inclusive scan, we need to apply each element's transform
    S_all = torch.einsum('bhlij,bhljk->bhlik', A, B_tree) + b_mat

    # Handle initial state
    if initial_state is not None:
        A_prefix = A_tree  # This would need recalculation for initial state
        S_all = torch.einsum('bhlij,bhjk->bhlik', A_prefix, initial_state.unsqueeze(2)) + S_all
...
def delta_rule_chunkwise_blelloch_scan_native_dtype(q, k, v, beta, chunk_size=32):
    """
    Hybrid chunkwise + Blelloch parallel scan, native dtype version.

    Uses Blelloch scan (O(n) work) instead of Hillis-Steele (O(n log n) work)
    for inter-chunk state propagation.
    """
    dtype = q.dtype
    device = q.device
    b, h, l, d_k = q.shape
    ...
    # Compute per-chunk A and B
    A_chunks = I.unsqueeze(0).unsqueeze(0).expand(b, h, num_chunks, d_k, d_k).clone()
    A_chunks = A_chunks - torch.einsum('bhncd,bhnce->bhnde', k_chunks, w)
    B_chunks = torch.einsum('bhncd,bhnce->bhnde', k_chunks, u)

    # Blelloch scan
    A_tree = A_chunks.clone()
    B_tree = B_chunks.clone()

    # Up-sweep
    step = 1
    while step < num_chunks:
        indices = torch.arange(2 * step - 1, num_chunks, 2 * step, device=device)
        if len(indices) == 0:
            break
        src_indices = indices - step
    ...
    step = num_chunks // 2
    while step >= 1:
        indices = torch.arange(step - 1, num_chunks - 1, 2 * step, device=device)
        if len(indices) == 0:
            break

        for idx in indices.tolist():
            right_idx = idx + step
            if right_idx >= num_chunks:
                continue
            A_t = A_tree[:, :, idx].clone()
            B_t = B_tree[:, :, idx].clone()
            A_tree[:, :, idx] = A_tree[:, :, right_idx].clone()
    ...
    # B_tree contains exclusive prefix sums (state before each chunk)
    S_prev = B_tree
    ...
    # Compute final state: apply last chunk's transform to its incoming state
    S_final = torch.einsum('bhij,bhjk->bhik', A_chunks[:, :, -1], S_prev[:, :, -1]) + B_chunks[:, :, -1]
